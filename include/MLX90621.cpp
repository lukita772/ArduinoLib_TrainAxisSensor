...
void MLX90621::setConfiguration() {
	...
	switch (refreshRate) {
	case 0:
		Hz_LSB = 0b00111111; //0.5hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
	case 1:
		Hz_LSB = 0b00111110; //1hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
	case 2:
		Hz_LSB = 0b00111101; //2hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
	case 4:
		Hz_LSB = 0b00111100; //4hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
	case 8:
		Hz_LSB = 0b00111011; //8hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
	case 16:
		Hz_LSB = 0b00111010; //16hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
	case 32:
		Hz_LSB = 0b00111001; //32hz, 18bit res, normal op, muestreo continuo, op. normal
		break;
  case 128:
    Hz_LSB = 0b00110111; //128hz
    break;
  case 256:
    Hz_LSB = 0b00110110; //256hz
    break;
	default:
		Hz_LSB = 0b00111110; //1hz, 18bit res, normal op, muestreo continuo, op. normal
	}
	byte defaultConfig_H = 0b01000110;  //kmoto: See data sheet p.11 and 25  (256hz, 15bit res, step mode, sleep mode..)
